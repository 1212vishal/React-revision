    ...
    return resList.length===0?<Shimmer/>:(
        <div className="body">
            ...
            <div className="search">
             ...
             <input className="Serach-box" type="Serach Resturant"  value={searchText} onChange={(e)=>{
                setsearchText(e.target.value);
             }}/>

             <button className="Fiter-Resturant" onClick={()=>{
                const filterResList=resList.filter((res)=>res.info.name.toLowerCase().includes(searchText.toLowerCase()));
            ...
            <div className="card-items">
              {
                dupResList.map((resturant)=>(<ResturantCard key={resturant.info.id} resData={resturant}/>))
              }
            </div>
        </div>
    )
